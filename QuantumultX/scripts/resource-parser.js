...
function HPS2QX(subs, Ptfo, Pcert0, PTls13) {
  ...
  if (server != '') {
    if (server.indexOf('@') != -1) {
      var ipport =
        'http=' + server.split('@')[1].split('#')[0].split('/')[0].split('?')[0]
...
function VR2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (subs.indexOf('alterId=') != -1) {
    caead =
      Number(subs.split('alterId=')[1].split('&')[0]) != 0 ? 'aead=false, ' : ''
  }
  ...
  if (typeU == 'SR-URI') {
    ...
    thost =
      cnt.indexOf('&host=') == -1
        ? thost
        : 'obfs-host=' +
          decodeURIComponent(cnt.split('&host=')[1].split('&')[0].split('#')[0])
    puri =
      cnt.indexOf('&path=') == -1
        ? puri
        : 'obfs-uri=' +
...
function SCT2QX(content) {
  ...
  var pudp =
    paraCheck(cnt, 'udp-relay') == 'true' ? 'udp-relay=true' : 'udp-relay=false'
  var nserver =
    pobfs != ''
